<x-app-layout>
    <!-- Hero Section -->
    <section id="hero">
        <!-- Flex Container -->
        <div class="container flex flex-col-reverse items-center px-6 mx-auto mt-10 space-y-0 md:space-y-0 md:flex-row">

            <div class="w-full mb-6">
                <img class="" src="{{asset('images/map/FloodBicolStressProneArea.png')}}" alt="" />

                <div class="hidden sm:block" aria-hidden="true">
                    <div class="py-5">
                        <div class="border-t border-gray-200"></div>
                    </div>
                </div>

                <img class="" src="{{asset('images/map/BicolStressProneArea.png')}}" alt="" />

                <div class="hidden sm:block" aria-hidden="true">
                    <div class="py-5">
                        <div class="border-t border-gray-200"></div>
                    </div>
                </div>

                <img class="" src="{{asset('images/map/AlbayStressProneArea.png')}}" alt="" />

                <div class="hidden sm:block" aria-hidden="true">
                    <div class="py-5">
                        <div class="border-t border-gray-200"></div>
                    </div>
                </div>

                <img class="" src="{{asset('images/map/CamSurStressProneArea.png')}}" alt="" />

                <div class="hidden sm:block" aria-hidden="true">
                    <div class="py-5">
                        <div class="border-t border-gray-200"></div>
                    </div>
                </div>

                <img class="" src="{{asset('images/map/CanamanStressProneArea.png')}}" alt="" />

                <div class="hidden sm:block" aria-hidden="true">
                    <div class="py-5">
                        <div class="border-t border-gray-200"></div>
                    </div>
                </div>

                <img class="" src="{{asset('images/map/TiwiStressProneArea.png')}}" alt="" />

            </div>

        </div>
    </section>
</x-app-layout>